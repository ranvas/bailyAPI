    ...
    public static class Api
    {
        ...
        public static string getOperators()
        {
            ...
            try
            {
                ...
                //Берем только те операторы, которые фигурируют в таблицах Operators и operatorCountries
                string dbCommand = "SELECT DISTINCT o.OperatorKey, o.Operator FROM Operators as o, OnLine.dbo.operatorCountries as oc where o.OperatorKey = oc.OperatorKey";
                //Открываем соединение, выполняем команду, записываем данные из таблицы в reader
                openSqlConnectionWithCommandExecution(ref con, out reader, dbCommand);
                ...
                {
                    dbCommand = "SELECT * from OnLine.dbo.operatorCountries where operatorKey=" + oper[0];
                    reader.Close();
                    //Открываем соединение, выполняем команду, записываем данные из таблицы в reader
                    openSqlConnectionWithCommandExecution(ref con, out reader, dbCommand);
